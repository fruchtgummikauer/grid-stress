...
### `data/metrics/smard_*.csv` — SMARD forecast errors (derived, planned)

Not created yet: written by `notebooks/02_forecast_metrics/forecast-metrics-claude.ipynb` once
[04-forecast-metrics.md](.claude/specs/04-forecast-metrics.md) is run. The `smard_` prefix marks
them as the baseline to beat, so our own model's files can sit next to them later.
...
- `smard_forecast_errors_hourly.csv` — one row per hour: actual, forecast and `err_*`
  ...
  The source of truth: any evaluation window, including the modelling spec's test window, is
  re-scored from this file.
- `smard_forecast_errors_daily.csv` — one row per calendar day: per-pair MAE / RMSE / bias /
  `hour_count`, plus residual load's day max/min errors and timing offsets.
- `smard_benchmark_metrics.csv` — long format (`level`, `period`, `pair`, `metric`, `value`,
  `hour_count`) for the cascade above day level plus the season slice.
...
| [04-forecast-metrics.md](.claude/specs/04-forecast-metrics.md) | Full spec, not yet run. Regression benchmark of SMARD's day-ahead forecasts (MAE, RMSE, bias, nMAE, `hour_count`) on `data/smard.csv` alone → `notebooks/02_forecast_metrics/forecast-metrics-claude.ipynb` plus the three `data/metrics/smard_*.csv` exports. |
